...
app.get('/api/attendance/status/:username', (req, res) => {
  ...
  if (!status) {
    ...
    // If clocked in, calculate elapsed seconds live
...
app.post('/api/attendance/clock-in', (req, res) => {
  ...
  const fenceRadius = 200; // 200 meters allowed radius
  ...
  if (location === 'Office') {
    ...
    if (distance > fenceRadius) {
      return res.status(400).json({
        success: false,
        message: `Clock-in Blocked: You are outside the office geofence. Distance: ${Math.round(distance)}m (Allowed limit: ${fenceRadius}m). Please select WFH or check in from the office.`
      });
    }
  }
...
app.post('/api/attendance/clock-out', (req, res) => {
  ...
  if (firstPunchTime) {
    ...
    if (totalMinutes > (9 * 60 + 30)) { // Late if check-in > 09:30 AM
      dayStatus = 'Late';
    }
  }
